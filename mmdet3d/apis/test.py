    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            ...
            if i <= 100:
                ...
                save_dir = './vis_target_sparse/'+f"{scene_dir}/"+f"{sample_dir}/"
                vis_target = np.uint8(result[0]['pred_occupancy'])
                #mask = np.uint8(result[0]['pred_cam_mask'])
                os.makedirs(save_dir, exist_ok=True)
                gt_target = data['gt_occupancy'][0][0]
                np.save(save_dir + f'cls_free_freeze_cam_0.7_learnable_dense_pred.npy', vis_target)
                #np.save(save_dir + f'cam_mask.npy', mask)
