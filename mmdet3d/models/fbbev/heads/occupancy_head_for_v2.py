...
class OccHead(BaseModule):
    def __init__(
        ...
        self.loss_feature_alignment_weight = self.loss_weight_cfg.get('loss_feature_alignment_weight', 1.0)
        self.loss_query_div_weight = self.loss_weight_cfg.get('loss_query_div_weight', 1.0)
    ...
    def loss_voxel(self, output_voxels, target_voxels, cam_mask, tag, **kwargs):
        ...
        # loss_dict['query_loss_{}'.format(tag)] = self.loss_query_div_weight * query_diversity_loss(kwargs['results']['global_queries'], kwargs['results']['attn_mask']) \
        #                                         + self.loss_feature_alignment_weight * hard_feature_query_alignment_loss(kwargs['results']['feature_map'], kwargs['results']['global_queries'],  kwargs['results']['attn_mask'])
         # Check Free area
        ##### 
        # Check Occluded Area
